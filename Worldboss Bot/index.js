...
module.exports = function Worldbossdiscord(mod) {
	...
	//	console.log(event)	
		...
		if(msg.id === 'SMT_FIELDBOSS_APPEAR')
		{
			...
			bams[name] = "Alive";
			discord.send("Worldboss " + name + " appeared");
		}
		else if(msg.id === 'SMT_FIELDBOSS_DIE_GUILD')	
		{
			...
			changed = true;
			let name = getBamName(msg.tokens.npcname);
			discord.send("Worldboss " + name + " was " + "killed" + " by " + msg.tokens.userName + ' from Guild '+msg.tokens.guildName);
		...
		else if(msg.id === 'SMT_FIELDBOSS_DIE_NOGUILD')
		{
			...
			changed = true;
			let name = getBamName(msg.tokens.npcname);
			discord.send("Worldboss " + name + " was " + "killed" + " by " + msg.tokens.userName);
			bams[name] = Date.now() + 5*60*60000 + (30 * 60 * 1000);
			discord.send(name + ": " + makeText(bams[name]));
		}
	});
	...
	function getBamName(id)
	{
		switch (id)
		{
			...
			case "@creature:39#501": return "Hazzard";
			default: return "John Cena";
		}
	}
	command.add('bamtime', () => {
		discord.send("Ortan: " + makeText(bams.Ortan));
		discord.send("Cerrus: " + makeText(bams.Cerrus));
		discord.send("Hazzard: " + makeText(bams.Hazzard));
	})
...
function makeText(date)
{
	...
	return "Respawns between " + (addZero(startT.getHours()) + ":" + addZero(startT.getMinutes())) + " - " + (addZero(endT.getHours()) + ":" + addZero(endT.getMinutes()));
}
